...
var ObjArr = JSON.parse(fs.readFileSync(path.resolve(__dirname, './utl.json'), 'utf-8'))
// ObjArr = 
function getUrlObj(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        let pageList = data[i].pageList;

        for (let j = 0; j < pageList.length; j++) {
            let actionList = pageList[j].actionList;
            // console.log(actionList)
            for (let k = 0; k < actionList.length; k++) {
                var item = actionList[k];
...
// (function () {
//     const directory = path.resolve(__dirname, './rap-data');
//     var arr = fs.readdirSync(directory)
//     // console.log(arr)
//     arr.map((jsonpath) => {
//         if (jsonpath != '.DS_Store') {
//             var data = JSON.parse(fs.readFileSync(path.resolve(__dirname, './rap-data/' + jsonpath), 'utf-8'));
//             //    console.log(  data.projectData.moduleList||[],"---")
//             //    console.log(  ,"ppppp")
//             urlArr = urlArr.concat(getUrlObj(data.projectData.moduleList));


//         }

//     })
//     fs.writeFileSync(`./utl.json`, JSON.stringify(urlArr))
...
htmlList.forEach(filePath => {
    ...
    fs.readFile(filePath, 'utf-8', function (err, data) {
        if (err) {
            ...
            // console.log($modules.length,"kkkk")
            for (let i = 0; i < $modules.length; i++) {
                ...
                (function ( j) {
                    ...
                    setTimeout(() => {
                        ...
                        fs.writeFile(`./api-json/${arr[arr.length - 1]}.json`, JSON.stringify(moduleData),function(err){
                            console.log(err)
                        })
                        console.log(j, "ok")
                    }, j * 300)
...
const compileAction = (page) => {
    ...
    for (let i = 0; i < $actions.length; i++) {
        ...
        actionData.push({
            "title": $h3.eq(0).text(),
            "path": '/' + $div.eq(1).text(),
            "method": getmethod($div.eq(1).text(), ObjArr),
            "req_body_type": "json",
            "res_body_type": "json",
